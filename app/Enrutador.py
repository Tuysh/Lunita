...
class Enrutador:
    def __init__(self):
        ...
        self.fallback_handler = (
            lambda ctx: "No entendí bien, ¿puedes decirlo de otra forma?"
        )
    ...
    def train(self):
        ...
        vectorizer = TfidfVectorizer(
            preprocessor=normalize, ngram_range=(1, 2), min_df=1
        )
        base_svm = LinearSVC()
    ...
    def load(self, path="intent_router.joblib"):
        ...
        self.regex_rules = [
            (re.compile(p, re.I), i) for p, i in data.get("regex_serialized", [])
        ]
        self._X = data.get("X", [])
        self._y = data.get("y", [])
    ...
    def route(
        ...
        try:
            ...
            top_margin = (
                float(decision[top_idx])
                if decision.ndim == 1 or decision.shape[0] == len(classes)
                else 0.0
            )
        except Exception:
            top_margin = 0.0
        ...
        if self._tfidf_matrix is not None:
            ...
            denom = np.linalg.norm(
                self._tfidf_matrix.toarray(), axis=1
            ) * np.linalg.norm(vec.toarray())
        ...
        if max_cos < SIM_MIN and max_cos_same_class < SIM_MIN:
            return self._default(
                text, reason=f"low_sim max={max_cos:.3f} class={max_cos_same_class:.3f}"
            )
    ...
    def _dispatch(self, intent, text, score):
        ...
        return type(
            "RouteResult", (), {"intent": intent, "score": score, "output": out}
        )

    def _default(self, text, reason=""):
        out = self.fallback_handler(
            {"text": text, "intent": "default", "score": 0.0, "reason": reason}
        )
        return type(
            "RouteResult", (), {"intent": "default", "score": 0.0, "output": out}
        )
